...
- Motion: 현재 위치와 도전 가능 상태만 절제해 움직이며 `prefers-reduced-motion`을 존중한다.
...
- Layout adaptations: 지도 노드와 배지 크기를 줄이되 첫·마지막 노드 전체가 지도 안에 남도록 한다.
...
- Success: 퀴즈 등록, 지역 수집 완료, 배지 획득, 도감 마스터를 서로 다른 단계로 표현한다.
- Disabled: 잠긴 지도 노드는 회색 처리와 `예고` 문구를 함께 사용한다.
- Preview/navigation: 지역 미리보기의 주 행동은 전체 지도로 돌아가 탐험 순서를 확인하며, 미완료 지역을 순서보다 앞서 선택하면 이동 확인 팝업을 표시한다.
