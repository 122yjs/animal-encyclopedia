...
async function main() {
  ...
  console.log('[record] shuffle 고정 완료');
  ...
  // Step 3: 퀴즈 정답 + "다음 문제"/"결과 보기" 모두 클릭
  for (let i = 0; i < 3; i++) {
    ...
    // 모든 문제에서 "다음 문제" 또는 "결과 보기" 클릭
    // 마지막 문제의 "결과 보기"를 클릭해야 finishQuiz() → showCatchAnimation() 호출
    await page.waitForSelector('.next-button', { timeout: 10000 });
    await page.locator('.next-button').click({ force: true, delay: 150 });
    console.log(`[record] ${i < 2 ? '다음 문제' : '결과 보기'} 클릭`);
    await page.waitForTimeout(800);
  }
  ...
  // Step 4: 닫기 버튼 클릭
  await page.screencast.showActions();
  await page.waitForSelector('#closeDetail', { timeout: 5000 });
  await page.locator('#closeDetail').click({ force: true, delay: 200 });
  console.log('[record] 닫기 버튼 클릭');
  await page.waitForTimeout(1500);
