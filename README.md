...
교사가 학생에게 공유할 때는 기본 GitHub Pages 주소를 그대로 QR로 만들면 질문방이 연결되지 않습니다. 반드시 `질문방 설정`에서 생성되는 수업용 도감 링크나 QR을 학생에게 공유해야 합니다.
...
5. 생성된 `학생에게 보낼 수업용 도감 링크` 또는 QR을 학생에게 공유합니다.
6. 학생이 수업용 링크나 QR로 접속하면 동물 상세창의 `더 궁금한 점 물어보기` 버튼이 해당 교사의 MagicSchool 방으로 연결됩니다.
